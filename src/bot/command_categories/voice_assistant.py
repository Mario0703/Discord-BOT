...
def register(
    bot, transcript_service, elevenlabs_service, voice_dir, transcripts_dir, guild_ids
):
    ...
    async def upload_mp3(
        ctx: discord.ApplicationContext, file: discord.Attachment, name: str
    ):
        ...
        if transcript_path is None:
            await MessageFormatting.send_response(
                ctx, "Please provide a valid transcript name."
            )
            return
        await MessageFormatting.send_response(
    ...
    async def eleven_labs(ctx: discord.ApplicationContext, text: str):
        ...
        try:
            ...
            audio_path = await asyncio.to_thread(
                elevenlabs_service.save_audio,
                audio,
                f"{datetime.now():%Y%m%d_%H%M%S_%f}.mp3",
            )
